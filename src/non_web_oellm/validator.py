...
class Validation:
    ...
    def validate_metadata(self, metadata):
        validated_metadata = {
            "LANGUAGE_CODE": self.validate_language_codes(
            ...
            "SOURCE_ORGANIZATION": self.validate_free_text(
            ...
            "MIXED_LANGUAGES": self.validate_mixed_languages(
                metadata.get("MIXED_LANGUAGES")
            ),
            "COMMENTARY": self.validate_free_text(metadata.get("COMMENTARY")),
            "DATASET_NAME_OR_DESCRIPTION": self.validate_free_text(
                metadata.get("DATASET_NAME_OR_DESCRIPTION")
            ),
            "DATE_OF_IDENTIFICATION": self.validate_date(
                metadata.get("DATE_OF_IDENTIFICATION")
            ),
            "CONTACT": self.validate_free_text(metadata.get("CONTACT")),
            "SOURCE_IDENTIFICATION_URL": self.validate_url(
                metadata.get("SOURCE_IDENTIFICATION_URL")
            ),
        ...
        if "NAME" in metadata:
            validated_metadata["NAME"] = metadata["NAME"]
        if "PATH" in metadata:
            validated_metadata["PATH"] = metadata["PATH"]
